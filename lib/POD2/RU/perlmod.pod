...
, то есть, после Perl завершения выполнения программы и перед переводчик
настоящее время выйденн, даже если это выход в результате функции die().
(Но если он перетекающей в другую программу через C <exec>, или
выдувается из воды по сигналу--вы должны в ловушку сами
(если вы можете).) Вы можете иметь несколько C <END>блоков в файле--они
будет выполняться в обратном порядке определения; то есть: последний, первый
вне (LIFO). C <END>блоки не выполняются при запуске perl с
C <-c> переключатель, или если происходит сбой компиляции.
