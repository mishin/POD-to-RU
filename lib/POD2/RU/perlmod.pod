...
Модули Perl всегда есть файл F<.pm>, но там также может быть
динамически связанные исполняемые файлы (часто заканчивается в F<.so>) или автозагрузки
подпрограмму определения (часто заканчивается в F<.al>), связанные с
модуль. Если это так, они будут полностью прозрачно для пользователя
модуль. Это ответственность на F<.pm> файл для загрузки
(или организовать автозагрузку) любые дополнительные функциональные возможности. Например,
Хотя модуль POSIX случается делать оба динамической загрузки и
Автозагрузка, пользователь может сказать просто C < использование POSIX >, чтобы получить все это.
