...
C<$bar>. На тот, который был набит независимо от C<local()> и, который будет
восстановлен, когда блок заканчится. Поскольку переменные доступны через
...
Что делает все это важно, что экспортер модуль использует шар
ступенчатость как механизм импорта/экспорта. Ли или не вы можете правильно
локализовать переменной, которые были экспортированы из модуля зависит как
