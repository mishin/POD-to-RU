...
Файл основной программы и каждого загружаемого ей модуля, это
единицы компиляции, такие как строка C<eval>s, во время выполнения кода, скомпилированного с помощью
C<(?{ })>  построить в regex, звонить C < ФАЙЛА >, C < требуют файл настроек,
и код после C <-e> переключателя командной строки.
