...
eval'ed строке); они будут выполнены в порядке определения. Потому что блок C<BEGIN>
выполняется немедленно  код блока, он может тянуть в определениях подпрограмм
и такое от других файлов во времени, чтобы быть видимым для остальной части компиляции
и во время выполнения. После того, как С <Начало> закончились, сразу определено и любой
Код раньше возвращается пула памяти в Perl.
