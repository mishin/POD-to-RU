...
отличаются друг от друга двумя способами. В первом случае, любой двойной
двоеточие в имени модуля, например C <Some::Module>, переводятся
в вашей системе каталогов разделитель, обычно «/». Второй
дело не и должны быть указаны буквально. Другой
разница что видя первый C <require>улики в компиляторе
использующий косвенное дополнение нотации с участием «SomeModule», как
в C < $ob = очистить SomeModule >, являются вызовы метода, а не функция.
(Да, это действительно можно сделать различие.)
