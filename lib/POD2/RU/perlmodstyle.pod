...
установите C<$Module::errstr> или подобное имя (C<errstr> как общее имя для ошибок, как это сделано
 в моделу DBI и других пополярных модулях; если вы выбете другое имя, то не забудьте
...
менее подробного до более детального. Ваша раздел СИНТАКСИСА(SYNOPSIS) должен содержать
...
Держите документации рядом с кодом документы ( "инлайн"
документация). Включите POD для данного метода прямо над что
подпрограммой метода. Это делает его легче держать документацию вверх
до настоящего времени, и позволяет избежать необходимости документировать каждый фрагмент кода дважды (один раз в
