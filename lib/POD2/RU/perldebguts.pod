...
=for regcomp.pl begin
 ...
 #               зацепленный вместе с их "следующими" указателями, так
 #               приоритет предотвращает ничего, объединяемых в
 #               любые отдельные ветви. "Следующий" указатель последнего
 #               Филиал в моменты выбора, что после
 #               весь выбор. Это также, где в финале "далее"
 #               указатель # каждого отдельного точек ветвления; каждая ветвь
 #               начинается с узлом операнда узла филиала.
 
 Набор отделений, составляющих единственный выбор,
               зацепленный вместе с их "следующими" указателями, с тех пор
               предшествование предотвращает то, чтобы это было связанным к
               любое отдельное отделение.

"Следующий" указатель последнего
 ОТДЕЛЕНИЕ # в выборе указывает на вещь после
 # целый выбор. Это также где финал "затем"
 Указатель # каждый отдельные точки разветвления; каждое отделение
 # начинается с узла операнда узла ОТДЕЛЕНИЯ.
 
                 The set of branches constituting a single choice are
