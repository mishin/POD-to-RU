...
Первая строка показывает предварительно скомпилированную форму регулярного выражения. Второй
...
Таблица смещения / длины, обычно 4 + C <размер> * 8. Следующая строка показывает
Label I <id> первого узла, который выполняет сопоставление.
