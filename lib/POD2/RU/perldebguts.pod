...
файла, скомпилированного с Perl. То же самое верно для C<выполняемых> (C<eval>ed) строк, 
...
То же самое для выполняемых строках, которые содержат подпрограммы, или
которые в настоящее время выполняются.$ Файла для С <> Eval строк ред
Похоже, С <(Eval 34)>.
