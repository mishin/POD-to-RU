...
# Это не совсем узел, а оптимизировать кусок "длинный"
узел#. Для упрощения отладки, мы отмечаем его, как будто это был узел
Оптимизирован от заполнителя для сброса.

# Специальные операции с недвижимостью, которые не код в скомпилированной программе
# никогда не будет этого типа. Таким образом, он может использоваться в качестве значения флага, что
# никакие другие операции не видно. Конец используется аналогичным образом, в том, что конец
# узел не может быть оптимизирован. Так что конец предполагает "unoptimizable" и псевдо
# в смысле "не видел ничего, чтобы оптимизировать".
Псевдо от псевдо код для внутреннего использования.
