...
Выглядит нормально, после того, как мы сделала проверку синтаксиса (perl -c scriptname), 
...
за деревьями, мы выпили чашку кофе и попробовали другой подход. Что
...
и, наконец, 'B<h h>' (h-space-h) выведет вам все команды с описанием. Вот суммарный список команд:
