...
Это общие рекомендации о том, как писать документацию Perl POD документацию
 для скриптов и модулей, на основе общих руководящих принципах для написания хорошей страницы
 UNIX man (далее маны). Все эти руководящие принципы являются, конечно, необязательными, но следование им
сделает вашу документацию более последовательной по отношению к другой документации
в системе.
...
, когда ссылаются на документацю имодуля , поскольку не ясно, какой раздел
...
исторически написаны в довольно поразительные формате ВСЕ ВЕРХНЕМ РЕГИСТРЕ; это
...
Идексаторы страниц руководства часто очень разборчивы в формате этого
...
запятой и пробелом. Для модуля Perl просто дайте имя модуля. A
один тире и только один тире разделяет список программ или функции из описания. 
Не используйте разметку, такую как CE<lt>E<gt> или BE<lt>E<gt>
 где-лиюбо в этой строке. Функции не должна писаться с C<()> или тому подобное. 
