...
До релиза Perl 5 было трудно представить сложных структуры данных
, потому что все ссылки должны были быть символическими -- и даже тогда
было трудно ссылаться на переменную вместо записи таблицы символов.
...
автоматически освобождая вещь на которую ссылаются, когда ее счетчик ссылок
 становится равным нулю. (Счетчики ссылок для значений в собственн-referential или
циклических данных структуры не могут идти к нулю без помощи; см.
L < / «Циклические Ссылки» > для подробного объяснения.)
Если это происходит в качестве объекта, объект разрушаются. См.
L <perlobj>для больше об объектах. (В смысле, все в Perl является
Объект, но мы обычно резерва слово для ссылок на объекты,
официально «благословил» в пакет класса.)
