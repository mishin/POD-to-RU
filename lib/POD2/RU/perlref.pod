...
perlref - Perl ссылки и вложенные структуры  данных (Perl references and nested data structures)

=head1 ПРИМЕЧАНИЕ

Это полная документация обо всех аспектах ссылок.
Более коротко, учебник-введение в только основные функции 
смотрите L<perlreftut>.

=head1 ОПИСАНИЕ

До релиза Perl 5 было трудно представить сложные структуры данных
, потому что все ссылки должны были быть символическими -- и даже тогда
было трудно ссылаться на переменную вместо записи в таблице символов.
Perl теперь не только делает более легким использование символических ссылок на переменные,
но также позволяет вам иметь "жесткие" (hard) ссылки на любую часть данных или кода.
Любой скаляр может содержать жесткую ссылку. Поскольку массивы и хэши содержат
скалярные значения, можно теперь легко создавать массивы массивов, массивы хэшей,
хэши массивов, массивы хэшей функций, и так далее.

Жесткие ссылки умны -- они отслеживают подсчет ссылок для вас,
автоматически освобождая сущность на которую ссылаются, когда ее счетчик ссылок
 становится равным нулю. (Счетчики ссылок для значений в само-ссылающихся или
циклических структурах данных  не могут стать нулями без помощи; см.
L</"Циклические Ссылки"> для подробного объяснения.)
Если это происходит для ссылки в виде объекта, то объект уничтножается(destructed). См.
L<perlobj>, чтобы узначть больше об объектах. (По смыслу, все в Perl является
Объектом, но мы обычно резервируем это слово для ссылок на объекты, которые 
были официально "благословлены" ("blessed") в пакет класса.)
