...
Любая команда, не распознаваемые отладчиком непосредственно выполняется
(C <eval>имел) как Perl код в текущем пакете. (Отладчик
использует пакет DB для держать свои собственные сведения о состоянии).
