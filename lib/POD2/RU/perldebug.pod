...
perldebug - Отладка кода в Perl (Perl debugging)

=head1 ОПИСАНИЕ

Во-первых, вы пытались использовать ключ B<-w> (warnings предупреждения)?

Если вы новичок в отладке Perl кода, вам лучше сначала прочитать
L<perldebtut>, который представляет собой учебник по пользованию отладчика (debugger).

=head1 Отладчик Perl

Если вы вызываете Perl с ключом B<-d>, ваш скрипт начинает работать под
отладчиком исходного кода Perl(Perl source debugger). Это работает, как интерактивная среда Perl
, где выполнение команды отладчика позволяет изучить
исходный код, установить точки останова, получите трассировку стека, изменить значения
переменных и т.д. Это настолько удобно, что вы часто запускаете
отладчик сама по себе просто чтобы проверить Perl конструкции
интерактивно для того, чтобы увидеть, что они делают. Например:
X<-d>

    $ perl -d -e 42


В случае ошибки (под Windows)
...
SetConsoleMode failed, LastError=|6| at C:/Users/TOSH/Documents/GitHub/dwimperl-
windows/strawberry-perl-5.20.0.1-32bit-portable/perl/vendor/lib/Term/ReadKey.pm
line 309.
 at C:/Users/TOSH/Documents/GitHub/dwimperl-windows/strawberry-perl-5.20.0.1-32b
it-portable/perl/vendor/lib/Term/ReadLine/readline.pm line 1581.
        readline::readline("  DB<1> ") called at C:/Users/TOSH/Documents/GitHub/
dwimperl-windows/strawberry-perl-5.20.0.1-32bit-portable/perl/vendor/lib/Term/Re
adLine/Perl.pm line 11
        Term::ReadLine::Perl::readline(Term::ReadLine::Perl=ARRAY(0x1d1ffec), "
 DB<1> ") called at C:/Users/TOSH/Documents/GitHub/dwimperl-windows/strawberry-p
erl-5.20.0.1-32bit-portable/perl/lib/perl5db.pl line 7285
        DB::readline("  DB<1> ") called at C:/Users/TOSH/Documents/GitHub/dwimpe
rl-windows/strawberry-perl-5.20.0.1-32bit-portable/perl/lib/perl5db.pl line 1832

        DB::_DB__read_next_cmd(undef) called at C:/Users/TOSH/Documents/GitHub/d
wimperl-windows/strawberry-perl-5.20.0.1-32bit-portable/perl/lib/perl5db.pl line
 2745
...
В Perl, отладчик не является отдельной программой так, как это обычно бывает в
типичной скомпилированной среде. Вместо этого, флаг B<-d> сообщает компилятору, 
вставлять информацию об исходном коде в деревья разбора и передать их интерпретатору. 
Это означает, что код сначала должен правильно скомпилироваться для отладчика для работы в нем. 
Затем, когда интерпретатор запускается, он
предварительно загружает специальный файл библиотеки Perl, содержащий отладчик.

Программа будет остановливаться  I<прямо перед> первой исполняемой конструкцией
(но см. ниже относительно конструкций времени компиляции) и попросить вас
 ввести команды отладчика. Вопреки популярным ожиданий, когда
отладчик останавливается и показывает вам строку кода, он всегда отображает
строку, которую I<собирается> выполнить, а не ту, что он только что исполнил.

Любая команда, которая не распознается отладчиком непосредственно выполняется
(с помощью C<eval>), как Perl код в текущем пакете. (Отладчик
использует пакет DB для хранения своих собственных сведений о состоянии).

Обратите внимание, что с выполнением C<eval> связана неявная область (implicit scope). Как
результат любые новые лексические переменные или изменение
содержимое буфера захвата (capture buffer content) теряется после eval. Отладчик предоставляет
удобное окружение (nice environment) для изучения Perl, но если ваш интерактивный эксперимент использует
материал, который должен быть в той же области(same scope), то тогда писать его нужно в одну строку.

Для любого текста, введенного в командной строке отладчика, начальные и конечные пробелы
сначала удаляется перед дальнейшей обработкой. Если команды отладчика
совпадает с некоторыми функциями в вашей собственной программе, просто перед
функцией поставьте то, что не выглядит как команды отладчика, такие
как ведущий C<;> или возможно C<+>, или путем заключения в кругые или фигурные скобки.

=head2 Вызов отладчика (Calling the Debugger)

Существует несколько способов вызова отладчика:

=over 4

=item perl -d program_name

В данной программе определяется C<program_name>.

=item perl -d -e 0 

Интерактивно укажите произвольное C<выражение>, с помощью C<-e>.

=item perl -d:Ptkdb program_name

Отладки данной программы через C<Devel::Ptkdb> GUI.

=item perl -dt threaded_program_name

Отладка программы с использованием потоков (экспериментальная возможность).

=back

=head2 Команды отладчика

Интерактивный отладчик распознает следующие команды:
...
Выводит справку для данной команды отладчика.

=item h h

Специальный аргумент C<h h> выводит всю страницу помощи, которая достаточно длинная.

Чтобы вывод команды C<h h> (или любой команды, коль на то пошло) можно было удобно просмотреть
 нужно поставить в начале трубу (pipe), тогда он пошлет документацию на программу просмотра (пейджер), как в

    DB> |h h

Вы можете изменить пейджер, который используется через  команду C<o pager=...>.
